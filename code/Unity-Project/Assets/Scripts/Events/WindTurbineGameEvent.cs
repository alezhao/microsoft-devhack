...
public class WindTurbineGameEvent : ScriptableObject
{
    ...
    public static ConcurrentQueue <WindTurbineScriptableObject> historicalData =new ConcurrentQueue<WindTurbineScriptableObject>();

    public void Raise(WindTurbineScriptableObject turbineData)
    {
        historicalData.Enqueue (turbineData);
        while (historicalData.Count>50)
        {
            historicalData.TryDequeue(out _);
        }
